...
class _TicketOrderScreenState extends State<TicketOrderScreen> {
  ...
  final int _maxTotalTickets =
      4; // Max 4 tickets across all categories for the user
    ...
    setState(() {
      ...
      if (totalAfterChange > _maxTotalTickets) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Maximum $_maxTotalTickets tickets allowed across all categories!')),
        );
        return;
      }
  ...
  Widget _buildAdjustableOrderItem(String category, double price, int quantity,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('RP${(price * quantity / 1000).toStringAsFixed(0)}K',
                    style: Theme.of(context)
                        .textTheme
                        .bodyMedium
                        ?.copyWith(color: Colors.white54)),
            ...
            children: [
              IconButton(
                icon: Icon(Icons.remove_circle_outline,
                    color:
                        quantity > 0 ? const Color(0xFF00D4FF) : Colors.grey),
                onPressed: quantity > 0 ? onDecrement : null,
              ),
              Text('$quantity', style: Theme.of(context).textTheme.bodyLarge),
              IconButton(
                icon: Icon(Icons.add_circle_outline,
                    color: _getTotalTicketCount() < _maxTotalTickets
                        ? const Color(0xFF00D4FF)
                        : Colors.grey),
                onPressed: _getTotalTicketCount() < _maxTotalTickets
                    ? onIncrement
                    : null,
              ),
            ],
        ...
        children: [
          Column(
            ...
            children: [
              ...
              Text('RP${(_calculateTotal() / 1000).toStringAsFixed(0)}K',
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: (_calculateTotal() > 0 &&
                        _selectedPaymentMethod != 'Change Payment Method')
                    ? Colors.blue
                    : Colors.grey,
              ),
              onPressed: (_calculateTotal() > 0 &&
                  ...
                  ? () async {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                            content: Text(
                                'Payment via $_selectedPaymentMethod successful!')),
                      );
