...
class ConcertWithTicket {
  ...
  factory ConcertWithTicket.fromMap(Map<String, dynamic> map) {
    // Sum filled and availability for all ticket categories
    int filled = 0;
    int availability = 0;
    if (map['concert_ticket'] != null && map['concert_ticket'] is List) {
      for (final ticket in map['concert_ticket']) {
        filled += (ticket['filled'] ?? 0) as int;
        availability += (ticket['availability'] ?? 0) as int;
      }
    }
